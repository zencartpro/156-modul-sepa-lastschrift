 ...
 * @version $Id: sepa-check.php 2019-09-12 19:02:14 webchills $
*/
...
// Diese Funktion gibt die Bankinformationen aus der Datenbank zurück*/
  function db_query($blz) {
  	global $db;
    $blz_query = $db->Execute("SELECT * from ".TABLE_SEPALASTSCHRIFT_BLZ." WHERE blz = '" . zen_db_input($blz) . "'");
    if ($blz_query->RecordCount() > 0) {
    
	$data = array (	'blz' => $blz_query->fields['blz'],
					'bankname' => $blz_query->fields['bankname'],
					'bic' => $blz_query->fields['bic'],
...
/* -------- Dies ist die wichtigste function ---------- */
  function CheckAccount($sepalastschrift_number, $sepalastschrift_blz) {
    $KontoNR = preg_replace('/[^0-9]/', '', $sepalastschrift_number); // Hetfield - 2009-08-19 - replaced deprecated function ereg_replace with preg_replace to be ready for PHP >= 5.3
    $BLZ = preg_replace('/[^0-9]/', '', $sepalastschrift_blz); // Hetfield - 2009-08-19 - replaced deprecated function ereg_replace with preg_replace to be ready for PHP >= 5.3
...
/*     Beginn Implementierung */
    ...
    if ($adata == -1) {
      ...
      $PRZ = -1;
      $this->PRZ = $PRZ;
      $this->sepalastschrift_number=ltrim($sepalastschrift_number,"0");
      $this->sepalastschrift_blz=$sepalastschrift_blz;
    } else {
      $this->Bankname = $adata['bankname'];
      $this->PRZ = str_pad ($adata['prz'], 2, "0", STR_PAD_LEFT);
      $this->sepalastschrift_number=ltrim($sepalastschrift_number,"0");
      //$this->sepalastschrift_number=$this->ExpandAccount($sepalastschrift_number);
      $this->sepalastschrift_blz=$sepalastschrift_blz;
...
/*                                                                               */
		...
		function __construct() {
			...
			$this->sepalastschrift_iban = ''; // Enthält nach der Prüfung die geprüfte IBAN
			$this->sepalastschrift_bic = '';  // Enthält nach der Prüfung den geprüften BIC
		...
		function IbanCheckAccount($iban, $bic = null) {
			...
			$this->sepalastschrift_iban = $this->iban_trim($iban);
			$this->sepalastschrift_bic = $this->bic_trim($bic);
    ...
    function bic_check_length($bic) {
		...
		global $db;
			...
			$result = 0;
					...
					$german_bank = $db->Execute("select * from ".TABLE_SEPALASTSCHRIFT_BLZ." where bic = '".zen_db_input($bic)."' or bic = '".zen_db_input($bic_padded)."' limit 1");	
					if ($german_bank->RecordCount() > 0) {
		...
		function message($error_code) {
 		...
 		function get_store_ctryiso2() {
 			global $db;
 			$country = $db->Execute("select countries_iso_code_2 from ".TABLE_COUNTRIES." where countries_id = '".(int)STORE_COUNTRY."'");
